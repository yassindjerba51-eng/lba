...
import { updateCompetencesPreviewSection, CompetencesPreviewData } from "@/app/actions/competencesPreview";
import { useRouter } from "next/navigation";
import { useLanguages } from "@/lib/LanguagesContext";

interface Props {
  initialData: CompetencesPreviewData;
}

export default function CompetencesPreviewManager({ initialData }: Props) {
  const [formData, setFormData] = useState<CompetencesPreviewData>(initialData);
  ...
  function updateField(field: keyof CompetencesPreviewData, locale: string, value: string) {
    setFormData((prev) => ({
      ...prev,
      [field]: {
        ...prev[field],
  ...
  async function handleSave() {
    startTransition(async () => {
      setMessage(null);
      const result = await updateCompetencesPreviewSection(formData);
      if (result.success) {
        setMessage({ type: "success", text: "Section Domaines d'Intervention mise à jour !" });
        router.refresh();
      } else {
        ...
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-lg flex items-center gap-2">
              <Layers className="h-5 w-5 text-primary" /> Nos domaines d&apos;intervention
            </CardTitle>
            <CardDescription>
              Gérez les textes et le lien d&apos;appel à l&apos;action de la section des secteurs (sous le CTA).
            </CardDescription>
          </div>
        </div>
      </CardHeader>
            ...
            return (
              <TabsContent key={locale} value={locale} className="space-y-4">
                <div>
                  ...
                  <Input
                    dir={dir}
                    placeholder={`Nos Domaines d'Intervention...`}
                ...
                <div>
                  ...
                  <Textarea
                    dir={dir}
                    placeholder={`Nous offrons des conseils juridiques spécialisés...`}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                  <div>
                    ...
                    <Input
                      dir={dir}
                      placeholder={`Voir tous les secteurs...`}
